...
test ('Create New USer', async({page})=>{
   ...
   /* //3.Click on add employees menu from the top menus & check name of the page
    ...
    await page.locator('[name="middleName"]').fill(mname)
    await page.locator('[name="lastName"]').fill(lname)

    await page.locator('(//input[@class="oxd-input oxd-input--active"])[2]').fill('1001231')
    await page.locator('//span[contains(@class,"oxd-switch-input oxd-switch-input--active")]').click()

    //create login details
    await page.locator('(//input[@class="oxd-input oxd-input--active"])[3]').fill(fname)
    ...
    await page.locator('(//input[@placeholder="Type for hints..."])[1]').fill(fname)
    await page.waitForTimeout(5000);
    await page.getByText(fname).click();
    await page.waitForTimeout(5000);
    await page.locator('//button[@type="submit"]').click();
    const newuser  = await page.locator('.oxd-table .oxd-table-body .oxd-table-cell:nth-of-type(3) div').textContent();
    console.log( newuser,  ': is presented in the table');
